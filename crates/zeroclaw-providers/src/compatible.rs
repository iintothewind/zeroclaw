...
///
/// Two independent idle bounds apply. `idle_timeout` is the streaming client's
/// read-idle bound, reported when a body read times out. `app_idle` is an
/// application-level deadline that only meaningful `data:` events refresh, so
/// SSE keep-alive comments cannot postpone stream finalization.
