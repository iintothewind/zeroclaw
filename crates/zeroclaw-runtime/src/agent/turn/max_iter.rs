...
mod graceful_summary_metering_tests {
    ...
    async fn graceful_summary_replays_signed_thinking_unmodified() {
        ...
        // Upstream's call site carries a separate attribution-model argument
        // plus the metering seam's token counter and observer; this fork's
        // `finish_after_max_iterations` predates that seam and takes neither,
        // so the call is trimmed to the fork's 14-parameter shape. The
        // assertion below is unaffected: it inspects what reached the provider,
        // not how the summary was metered.
        let out = finish_after_max_iterations(
            &provider,
            &mut history,
            "custom",
