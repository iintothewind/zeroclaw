...
messages whose images are dispatched: user turns and the tool results of the
current user turn. Markers that preparation strips from older tool results are
not charged, and system and assistant text is priced as text. After each
accepted response, `ContextCalibration` re-anchors on provider `input_tokens`;
optional `cached_tokens` is cache observability only (subset of prompt size for
