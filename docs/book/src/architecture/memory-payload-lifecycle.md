...
| History trimming | `agent::history` and `agent::history_trim` | Lossy change to durable session / agent history **and** the provider request | Trimming must be visible, rewrite session storage, preserve tool-call/tool-result pairing, refresh context metering, and avoid pretending old context is still available. |
